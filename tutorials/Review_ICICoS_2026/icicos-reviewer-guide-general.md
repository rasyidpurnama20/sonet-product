...
> **Versi:** 1.0 · **Berlaku untuk:** ICICoS 2026 dan seterusnya  
...
## BAGIAN A — DEKLARASI AWAL (WAJIB DIISI SEBELUM MEMULAI REVIEW)
...
--- Deklarasi Konflik Kepentingan (COI) ---
coi_status        : [ ] Tidak ada konflik kepentingan
                    [ ] ADA konflik — saya mengundurkan diri dari review ini
Konflik yang dimaksud: kolaborasi dalam 3 tahun terakhir, hubungan institusi,
kompetisi langsung, atau hubungan personal dengan penulis.

--- Deklarasi Penggunaan AI ---
ai_digunakan      : [ ] Tidak  [ ] Ya
tool_ai           : [nama tool jika Ya — pastikan tool tidak melatih model dari input]
peran_ai          : [misal: merapikan bahasa komentar / brainstorming pertanyaan]
catatan           : Keputusan Accept/Reject TETAP tanggung jawab reviewer, bukan AI.
```
...
Centang setiap item yang *sudah* diperiksa. Item yang gagal → buat komentar di Bagian D.

### C.1 Abstract
- [ ] Klaim utama sesuai dengan hasil yang dilaporkan
- [ ] Angka/metrik kunci disebutkan secara eksplisit
- [ ] Kontribusi dinyatakan dengan jelas dan spesifik

### C.2 Introduction & Motivasi
- [ ] Masalah didefinisikan dengan jelas dan urgensinya terjustifikasi
- [ ] Kontribusi spesifik dan dapat diuji/diverifikasi
- [ ] Struktur paper dijelaskan di akhir Introduction

### C.3 Related Work
- [ ] Gap penelitian diidentifikasi secara eksplisit
- [ ] Sitasi terkini dan mencakup karya fundamental
- [ ] Posisi kontribusi paper ini terhadap literatur jelas

### C.4 Data & Preprocessing *(jika applicable)*
- [ ] Sumber data dijelaskan (asal, ukuran, lisensi/izin akses)
- [ ] Atribut dan label didefinisikan secara operasional
- [ ] Prosedur preprocessing dilaporkan (imputation, normalisasi, split)
- [ ] Risiko *data leakage* tidak teridentifikasi

### C.5 Metodologi & Model
- [ ] Pilihan algoritma/arsitektur dijustifikasi, bukan sekadar disebutkan
- [ ] Baseline yang relevan disertakan sebagai pembanding
- [ ] Hyperparameter dilaporkan dengan lengkap
- [ ] Strategi validasi jelas (cross-validation, hold-out, dll.)

### C.6 Eksperimen & Hasil
- [ ] Metrik evaluasi sesuai dengan karakteristik data (misal: F1 untuk data tidak seimbang)
- [ ] Hasil disajikan dengan rata-rata ± standar deviasi lintas run
- [ ] Uji signifikansi statistik dilakukan (jika membandingkan antar-model)
- [ ] Tabel dan gambar terbaca, berlabel, dan direferensikan dalam teks

### C.7 Diskusi
- [ ] Hasil diinterpretasikan melampaui sekadar menyebut angka
- [ ] Limitasi penelitian diakui secara jujur
- [ ] Ancaman validitas (threats to validity) diidentifikasi

### C.8 Kesimpulan & Future Work
- [ ] Kesimpulan konsisten dengan hasil (tidak over-claim)
- [ ] Arah penelitian selanjutnya spesifik dan realistis

### C.9 Referensi & Format
- [ ] Semua referensi dalam daftar pustaka dirujuk dalam teks
- [ ] Format sitasi konsisten dengan panduan ICICoS
- [ ] Panjang paper sesuai batas halaman yang ditentukan

### C.10 Reproduksibilitas
- [ ] Kode / data tersedia atau ada pernyataan ketersediaan yang jelas
- [ ] Seed, versi library, dan lingkungan eksperimen dilaporkan
- [ ] Langkah-langkah cukup untuk mereplikasi hasil utama
...
*Gunakan untuk: bagian yang ambigu atau tidak lengkap, di mana reviewer butuh penjelasan sebelum menilai validitasnya — bukan klaim bahwa ada kesalahan.*
...
Pertanyaan: "Early process signatures" didefinisikan pada menit ke berapa setelah
            proses dimulai, dan apakah threshold ini ditentukan secara empiris atau
            berdasarkan domain knowledge?
Konteks   : Definisi operasional ini kritis untuk menilai replikabilitas dan
            potensi data leakage pada fitur yang dibentuk.
```
...
Kesenjangan : Studi [A, B, C] telah menggunakan pendekatan serupa di domain logistik
             lain; karya-karya ini tidak disitasi atau dibandingkan.
Rekomendasi : Tambahkan dan posisikan paper ini secara eksplisit terhadap karya
             tersebut untuk memperjelas kebaruan yang spesifik.
```
...
Elemen yang hilang : Sebutkan informasi spesifik yang tidak tersedia.
Dampak             : Jelaskan mengapa ketidaktersediaan ini menghambat verifikasi.
...
Elemen yang hilang : Random seed, versi library (scikit-learn, TensorFlow), dan
                    spesifikasi hardware tidak dilaporkan.
Dampak             : Hasil tidak dapat direplikasi secara independen, sehingga
                    klaim performa tidak dapat diverifikasi oleh pembaca.
Rekomendasi        : Tambahkan tabel konfigurasi eksperimen (seed, versi, hardware)
                    atau sertakan data/kode di repositori publik dengan pernyataan
                    ketersediaan di akhir paper.
```

---

#### Format #5 — Apresiasi (Kekuatan Paper)
*Gunakan untuk: mengakui kontribusi atau aspek yang benar-benar kuat. Minimal satu komentar apresiasi diperlukan per review untuk memastikan keseimbangan.*
...
Aspek  : Motivasi operasional masalah disampaikan dengan sangat jelas, dilengkapi
         data kuantitatif kerugian akibat pelanggaran dwelling time.
Alasan : Ini membantu pembaca dari berbagai latar belakang memahami urgensi
         masalah tanpa perlu pengetahuan domain khusus.
```
...
```
Saya menemukan masalah teknis/metodologis yang spesifik  → Format #1
Saya tidak mengerti sesuatu dan butuh penjelasan          → Format #2
Klaim kebaruan/sitasi tidak mencukupi                     → Format #3
Detail eksperimen tidak cukup untuk replikasi             → Format #4
Ada aspek yang genuinely kuat                             → Format #5
```

> **Tag pelacakan (opsional):** Setiap komentar dapat diberi tag
> `[ID] [Seksi] [Format#] [Severity] [Status: Open]`
> untuk memudahkan pelacakan antar-ronde revisi.

---

## BAGIAN E — RINGKASAN REVIEW

Isi setelah semua komentar selesai. Bagian ini yang dibaca editor program.

```
=== RINGKASAN REVIEW ===

Rekomendasi Akhir : [pilih dari Bagian B.1]
Keyakinan Reviewer: [skor 0–4 dari Bagian B.2]

--- Poin MAJOR (wajib direvisi) ---
1. [ringkasan singkat komentar MAJOR #1]
2. [ringkasan singkat komentar MAJOR #2]
...

--- Poin MINOR (disarankan direvisi) ---
1. [ringkasan singkat komentar MINOR #1]
...

--- Kekuatan Paper ---
1. [ringkasan kekuatan utama]
...

--- Komentar Bebas untuk Editor (opsional, tidak terlihat penulis) ---
[jika ada informasi yang perlu disampaikan ke editor saja]
```

---

## BAGIAN F — ATURAN ETIKA PENGGUNAAN AI
...
4. **AI diizinkan sebagai:** (a) asisten bahasa — merapikan nada dan tatabahasa komentar; (b) penstruktur — memastikan checklist lengkap; (c) brainstorming pertanyaan — tetapi reviewer wajib memverifikasi setiap poin terhadap isi paper.
5. **Disclosure wajib:** Jika AI digunakan, nyatakan di Bagian A (Deklarasi Penggunaan AI).
...
*Pertanyaan prosedural hubungi Program Chair melalui sistem submission.*
